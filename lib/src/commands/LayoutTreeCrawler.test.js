...
describe('LayoutTreeCrawler', () => {
  ...
  it('crawls a layout tree and adds unique id to native node', () => {
    const node = { type: LayoutTypes.Native, data: { name: 'Smth smth' }, children: [] };
    uut.crawl(node);
    expect(node.id).toEqual('Native+UNIQUE_ID');
  });

  it('crawls a bottomTabs layout tree with native children', () => {
    const node = { type: LayoutTypes.BottomTabs, children: [
      { type: LayoutTypes.Native, data: { name: 'Here I am' } }
    ] };
    uut.crawl(node);
    expect(node.id).toEqual('BottomTabs+UNIQUE_ID');
    expect(node.children[0].id).toEqual('Native+UNIQUE_ID');
  });
